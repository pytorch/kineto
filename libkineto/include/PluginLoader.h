...
public:
    ...
    int registerPluginProfiler(const KinetoPlugin_ProfilerInterface *pProfiler)
    {
        ...
        libkineto::api().registerProfilerFactory([profiler]() -> std::unique_ptr<IActivityProfiler> {
...
private:
    ...
    static int cRegisterProfiler(KinetoPlugin_RegistryHandle *pRegistryHandle, const KinetoPlugin_ProfilerInterface *pProfiler)
    {
...
inline void loadPlugins()
{
    ...
    {
        LOG(VERBOSE) << "Environment variable " << pPluginLibDirPathEnvVar << " not set";
    ...
    {
        LOG(ERROR) << "Failed to unset environment variable " << pPluginLibDirPathEnvVar << " at unsetenv() with error " << strerror(errno);
    ...
    try 
    {
        for (const auto& entry : std::filesystem::directory_iterator(pPluginLibDirPath)) 
        {
    ...
    for (const auto& libFilePath : libFilePaths)
    {
        ...
        {
            char *pError = dlerror();
            LOG(WARNING) << "Failed to open " << libFilePath << " at dlopen() with error " << pError;
            continue;
        }

        int (*pfxRegister)(const KinetoPlugin_Registry *pRegistry) = reinterpret_cast<int(*)(const KinetoPlugin_Registry *pRegistry)>(dlsym(pHandle, "KinetoPlugin_register"));

        if (pfxRegister == nullptr)
        {
            char* pError = dlerror();
            LOG(VERBOSE) << "Failed to find symbol KinetoPlugin_register() from " << libFilePath << " at dlsym() with error " << pError;
        ...
        if (errorCode != 0)
        {
            LOG(ERROR) << "Failed to register plugin profiler from " << libFilePath << " at pfxRegister() with error " << errorCode;
        }
    }
