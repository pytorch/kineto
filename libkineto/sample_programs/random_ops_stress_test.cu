...
// Malloc / Free have different behaviors depending on cuda versions
#define CUDA_1102
...
void generate_tensor_cache(
  ...
  uint32_t num_pairs_estimated =
      sz_cache_KB / (3 * (sz_max_tensor_KB - sz_min_tensor_KB) / 2);
  ...
  for (int i = 0; i < num_pairs_estimated; ++i) {
    uint32_t num_KB =
        rand() % (sz_max_tensor_KB - sz_min_tensor_KB) + sz_min_tensor_KB;
    ...
    if (((float)(rand() % 10000) / 10000.0) < prob_h2d) {
    ...
    if (((float)(rand() % 10000) / 10000.0) < prob_d2h) {
    ...
    if (sz_memory_pool_KB >= sz_cache_KB) {
