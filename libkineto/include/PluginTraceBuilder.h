...
// Event builder provides a simple abstraction for plugins to interace with the event system
// in kineto. This works across binaries and has to be dealt with care.
// We do not use any C++ stdlib component across the interface and thus we need to translate a few things around
...
public:
    ...
    static inline ActivityType convertToActivityType(KinetoPlugin_ProfileEventType type)
    {
        switch (type) 
        {
    ...
    static inline unsigned int convertToLinkType(KinetoPlugin_ProfileEventFlowType type)
    {
        switch (type)
        {
    ...
    int addEvent(const KinetoPlugin_ProfileEvent *pProfileEvent)
    {
        ...
        if (pProfileEvent->unpaddedStructSize < KINETO_PLUGIN_PROFILER_PROCESS_EVENTS_PARAMS_UNPADDED_STRUCT_SIZE)
        {
    ...
    int setLastEventFlow(const KinetoPlugin_ProfileEventFlow *pProfileEventFlow)
    {
        ...
        // Handle versioning
        // Currently expect the exact same version
        if (pProfileEventFlow->unpaddedStructSize < KINETO_PLUGIN_PROFILE_EVENT_FLOW_UNPADDED_STRUCT_SIZE)
        {
    ...
    int addLastEventMetadata(const char *pKey,  const char *pValue)
    {
        ...
        if (buffer_->activities.empty())
        {
            LOG(ERROR) << "Failed to set last event metadata as there is no last event";

            return -1;
        }

        buffer_->activities.back()->addMetadata(std::string{pKey}, std::string{pValue});

        return 0;
    }

    int addResourceInfo(const KinetoPlugin_ProfileResourceInfo *pProfileResourceInfo)
    {
        ...
        // Handle versioning
        // Currently expect the exact same version
        if (pProfileResourceInfo->unpaddedStructSize < KINETO_PLUGIN_PROFILE_RESOURCE_INFO_UNPADDED_STRUCT_SIZE)
        {
        ...
        ResourceInfo resourceInfo(pProfileResourceInfo->deviceId, pProfileResourceInfo->resourceId, pProfileResourceInfo->displayOrder, pProfileResourceInfo->pName ? std::string(pProfileResourceInfo->pName) : std::to_string(pProfileResourceInfo->resourceId));
    ...
    const KinetoPlugin_TraceBuilder toCTraceBuilder()
    {
        return KinetoPlugin_TraceBuilder
        {
            ...
            .pTraceBuilderHandle = reinterpret_cast<KinetoPlugin_TraceBuilderHandle *>(this),
            .addEvent = cAddEvent,
...
private:
    static int cAddEvent(KinetoPlugin_TraceBuilderHandle *pTraceBuilderHandle, const KinetoPlugin_ProfileEvent *pProfileEvent) 
    {
        auto pPluginTraceBuilder = reinterpret_cast<PluginTraceBuilder*>(pTraceBuilderHandle);
        return pPluginTraceBuilder->addEvent(pProfileEvent);
    }

    static int cSetLastEventName(KinetoPlugin_TraceBuilderHandle *pTraceBuilderHandle, const char *name) 
    {
        auto pPluginTraceBuilder = reinterpret_cast<PluginTraceBuilder*>(pTraceBuilderHandle);
        return pPluginTraceBuilder->setLastEventName(name);
    }

    static int cSetLastEventFlow(KinetoPlugin_TraceBuilderHandle *pTraceBuilderHandle, const KinetoPlugin_ProfileEventFlow *pProfileEventFlow) 
    {
        auto pPluginTraceBuilder = reinterpret_cast<PluginTraceBuilder*>(pTraceBuilderHandle);
        return pPluginTraceBuilder->setLastEventFlow(pProfileEventFlow);
    }

    static int cAddLastEventMetadata(KinetoPlugin_TraceBuilderHandle *pTraceBuilderHandle, const char *pKey,  const char *pValue)
    {
        auto pPluginTraceBuilder = reinterpret_cast<PluginTraceBuilder*>(pTraceBuilderHandle);
        return pPluginTraceBuilder->addLastEventMetadata(pKey, pValue);
    }

    static int cAddResourceInfo(KinetoPlugin_TraceBuilderHandle *pTraceBuilderHandle, const KinetoPlugin_ProfileResourceInfo *pProfileResourceInfo) 
    {
        auto pPluginTraceBuilder = reinterpret_cast<PluginTraceBuilder*>(pTraceBuilderHandle);
