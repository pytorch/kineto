...
export const LineChart: React.FC<IProps> = (props) => {
  const { graph, height = 400, hAxisTitle, vAxisTitle, initialSelectionStart, initialSelectionEnd, onSelectionChanged } = props
  ...
  React.useLayoutEffect(() => {
    ...
    const state = {
      startX: cli.getXLocation(initialSelectionStart) - graphRect.left,
      endX: cli.getXLocation(initialSelectionEnd) - graphRect.left,
      mouseDown: false,
      minX: 0,
      maxX: 0
    }
    console.log([state, initialSelectionStart, initialSelectionEnd, graphRect.left])
  ...
  return (
    <div className={classes.root}>
      ...
      <div ref={rangeDivRef} style={{backgroundColor: 'transparent', position: 'relative', opacity: 0.3}}>
        <div ref={selectedDivRef} style={{backgroundColor: 'red', marginLeft: 0, width: 0}}></div>
      </div>
    </div>
  )
}
