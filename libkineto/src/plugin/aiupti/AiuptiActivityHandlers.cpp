...
inline bool
AiuptiActivityProfilerSession::outOfRange(const ITraceActivity &act) {
  bool out_of_range =
...
void AiuptiActivityProfilerSession::handleRuntimeActivity(
  ...
  traceBuffer_.emplace_activity(traceBuffer_.span, ActivityType::CUDA_RUNTIME,
                                cbIDName);
...
void AiuptiActivityProfilerSession::handleKernelActivity(
  ...
  traceBuffer_.emplace_activity(traceBuffer_.span,
                                ActivityType::CONCURRENT_KERNEL, name);
...
template <class memory_activity_type>
uint32_t
AiuptiActivityProfilerSession::getResourceId(memory_activity_type *activity) {
...
template uint32_t
AiuptiActivityProfilerSession::getResourceId<AIUpti_ActivityMemcpy>(
    AIUpti_ActivityMemcpy *activity);
template uint32_t
AiuptiActivityProfilerSession::getResourceId<AIUpti_ActivityMemory>(
    AIUpti_ActivityMemory *activity);
template uint32_t
AiuptiActivityProfilerSession::getResourceId<AIUpti_ActivityMemset>(
    AIUpti_ActivityMemset *activity);
