...
class GenericActivityProfiler {
 ...
 protected:
  ...
  virtual void logGpuVersions() = 0;
  virtual void setMaxGpuBufferSize(int size) = 0;
  virtual void enableGpuTracing() = 0;
  virtual void disableGpuTracing() = 0;
  virtual void clearGpuActivities() = 0;
  virtual bool isGpuCollectionStopped() const = 0;
  virtual void processGpuActivities(ActivityLogger& logger) = 0;
  virtual void synchronizeGpuDevice() = 0;
  virtual void pushCorrelationIdImpl(uint64_t id, CorrelationFlowType type) = 0;
  virtual void popCorrelationIdImpl(CorrelationFlowType type) = 0;
  virtual void onResetTraceData() = 0;
  virtual void onFinalizeTrace(
      const Config& config,
      ActivityLogger& logger) = 0;
