...
// Simple CSV logger for testing custom logger registration
class SimpleCsvLogger : public libkineto::ActivityLogger {
 public:
  explicit SimpleCsvLogger(const std::string& path)
      : path_(path), event_count_(0) {
    file_.open(path);
    file_
        << "event_num,name,type,timestamp_ns,duration_ns,device_id,resource_id,correlation_id\n";
  }

  ~SimpleCsvLogger() override {
    if (file_.is_open()) {
      file_.close();
    }
  }
  ...
  void handleActivity(const libkineto::ITraceActivity& activity) override {
    event_count_++;
    file_ << event_count_ << "," << activity.name() << ","
          << (int)activity.type() << "," << activity.timestamp() << ","
          << activity.duration() << "," << activity.deviceId() << ","
          << activity.resourceId() << "," << activity.correlationId() << "\n";
  }
  ...
  void finalizeTrace(
    ...
    if (file_.is_open()) {
      file_.flush();
      file_.close(); // Explicitly close to ensure all data is written
    }
  }
...
// Test custom CSV logger registration and integration
// WARNING: This test adds a csv logger to the global logger factory
TEST_F(CuptiActivityProfilerTest, CustomCsvLoggerIntegration) {
#ifdef __linux__
  char csvFilename[] = "/tmp/libkineto_csv_testXXXXXX.csv";
  createTempTraceFile(csvFilename, 4); // .csv has 4 characters

  char jsonFilename[] = "/tmp/libkineto_json_testXXXXXX.json";
  createTempTraceFile(jsonFilename);

  // Register CSV logger using the public API
  libkineto::registerLoggerFactory("csv", [](const std::string& path) {
    return std::make_unique<SimpleCsvLogger>(path);
  });
  ...
  // Create some CPU activities
  auto cpuOps = std::make_unique<MockCpuActivityBuffer>(
      start_time_ns, start_time_ns + duration_ns);
  cpuOps->addOp("test_op_1", start_time_ns + 10, start_time_ns + 50, 1);
  cpuOps->addOp("test_op_2", start_time_ns + 60, start_time_ns + 100, 2);
  cpuOps->addOp("test_op_3", start_time_ns + 110, start_time_ns + 200, 3);
  profiler.transferCpuTrace(std::move(cpuOps));
  ...
  // Save using the existing "file" protocol (baseline - should still work)
  trace.save(jsonFilename);
  checkTracefile(jsonFilename);

  // Save using our new "csv" protocol
  trace.save(std::string("csv://") + csvFilename);

  // Verify CSV file was created and has content
  std::ifstream csvFile(csvFilename);
  ASSERT_TRUE(csvFile.is_open()) << "Failed to open CSV file: " << csvFilename;
  ...
  // Should have header + at least 3 events (our 3 test ops)
  EXPECT_GE(lines.size(), 4) << "CSV should have header + at least 3 events";
  ...
  // Verify we have our test operations in the CSV
  std::string csvContent;
  for (const auto& l : lines) {
    csvContent += l + "\n";
  }
  EXPECT_NE(csvContent.find("test_op_1"), std::string::npos)
      << "CSV should contain test_op_1";
  EXPECT_NE(csvContent.find("test_op_2"), std::string::npos)
      << "CSV should contain test_op_2";
  EXPECT_NE(csvContent.find("test_op_3"), std::string::npos)
      << "CSV should contain test_op_3";

  // Verify both files were created (showing multi-format support)
  struct stat jsonStat, csvStat;
  EXPECT_EQ(stat(jsonFilename, &jsonStat), 0) << "JSON file should exist";
  EXPECT_EQ(stat(csvFilename, &csvStat), 0) << "CSV file should exist";
  EXPECT_GT(jsonStat.st_size, 100) << "JSON file should have content";
  EXPECT_GT(csvStat.st_size, 100) << "CSV file should have content";
