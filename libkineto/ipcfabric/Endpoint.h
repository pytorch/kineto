...
class EndPoint final {
 ...
 public:
  ...
  const char* getName(TCtxt const& ctxt) const {
    ...
    if (is_domain_socket) {
      ...
      if (strncmp(socket_dir, ctxt.msg_name.sun_path, socket_dirname_len) != 0 || ctxt.msg_name.sun_path[socket_dirname_len] != '/') {
        throw std::invalid_argument("Unexpected socket name: " + std::string(ctxt.msg_name.sun_path) + ". Expected to start with " + std::string(socket_dir) + "/");
      }
      return ctxt.msg_name.sun_path + socket_dirname_len + 1;
    } else {
      if (ctxt.msg_name.sun_path[0] != '\0') {
        throw std::invalid_argument("Expected abstract socket, got " + std::string(ctxt.msg_name.sun_path));
      }
