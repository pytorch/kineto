...
class ProtoToGvizTest(unittest.TestCase):
  ...
  def create_mock_run_environment(self):
    ...
    for _ in range(0, 3):
      ...
      jobs.append(job)
    run_env.host_dependent_job_info.extend(jobs)
  ...
  def test_run_environment_empty(self):
    ...
    self.assertEqual(len(data_table.columns), self.run_env_num_columns)
  ...
  def test_run_environment_simple(self):
    ...
    self.assertEqual(len(table_description), self.run_env_num_columns)
    self.assertEqual(len(data_table.columns), self.run_env_num_columns)
  ...
  def create_mock_overview_page_analysis(self):
    ...
    for _ in range(0, 3):
      ...
      ops.append(op)
    analysis.top_device_ops.extend(ops)
  ...
  def test_overview_page_analysis_empty(self):
    ...
    self.assertEqual(len(data_table.columns), len(list(self.mock_tf_op)))
  ...
  def test_overview_page_analysis_simple(self):
    ...
    self.assertEqual(3, data_table.NumberOfRows(), "Simple table has 3 rows.")
    # Check the number of columns in table descriptor and data table.
    self.assertEqual(len(table_description), len(list(self.mock_tf_op)))
    self.assertEqual(len(data_table.columns), len(list(self.mock_tf_op)))
  ...
  def test_recommendation_simple(self):
    ...
    self.assertEqual(len(data), len(list(self.mock_tips)))
    self.assertEqual(
        len(list(self.mock_tips)), data_table.NumberOfRows(),
