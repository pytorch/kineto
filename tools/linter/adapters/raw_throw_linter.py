...
def replacement_macro(path: str) -> str:
    ...
    # Kineto is a standalone library that cannot depend on c10, so it routes
    # every throw through its own KINETO_THROW (libkineto/src/ThrowUtil.h).
    # Matches both the standalone repo and third_party/kineto/ in PyTorch.
    if "libkineto/" in posix:
        return "KINETO_THROW"
    return "TORCH_CHECK"
