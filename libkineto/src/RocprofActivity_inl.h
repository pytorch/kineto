...
inline const std::string GpuActivity::name() const {
  if (type_ == ActivityType::CONCURRENT_KERNEL) {
    ...
    return demangle(raw().kernelName.length() > 0 ? raw().kernelName
                                                  : std::string(name));
    ...
    return fmt::format("Memcpy {} ({} -> {})",
                       getGpuActivityKindString(raw().domain, raw().op), src,
                       dst);
  } else {
    return "";
  }
...
template <class T> inline bool RuntimeActivity<T>::flowStart() const {
...
RuntimeActivity<rocprofKernelRow>::metadataJson() const {
  std::string kernel = "";
  ...
  return fmt::format(
      ...
      "block": [{}, {}, {}],
      "shared memory": {})JSON",
      kernel, raw().cid, raw().id, raw().gridX, raw().gridY, raw().gridZ,
      raw().workgroupX, raw().workgroupY, raw().workgroupZ,
...
inline const std::string RuntimeActivity<rocprofCopyRow>::metadataJson() const {
  ...
  return fmt::format(
      ...
      raw().cid, raw().id, raw().src, raw().dst, raw().size,
      fmt::underlying(raw().kind));
}

template <>
inline const std::string
RuntimeActivity<rocprofMallocRow>::metadataJson() const {
  correlationToSize[raw().id] = raw().size;
  std::string size = "";
