...
template <class T> struct RocprofActivity : public ITraceActivity {
...
template <class T> struct RuntimeActivity : public RocprofActivity<T> {
  ...
  const std::string name() const override {
    return RocprofLogger::opString(ROCPROFILER_CALLBACK_TRACING_HIP_RUNTIME_API,
                                   raw().cid);
  }
  void log(ActivityLogger &logger) const override;
  const std::string metadataJson() const override;
  const T &raw() const { return RocprofActivity<T>::raw(); }
};
