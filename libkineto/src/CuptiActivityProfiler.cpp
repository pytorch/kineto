...
template <> struct std::hash<CtxEventPair> {
...
#ifdef HAS_ROCTRACER
#ifndef ROCTRACER_FALLBACK
CuptiActivityProfiler::CuptiActivityProfiler(RocprofActivityApi &cupti,
                                             bool cpuOnly)
#else
...
void CuptiActivityProfiler::processTraceInternal(ActivityLogger &logger) {
  ...
  if (!cpuOnly_) {
    ...
    const int count = cupti_.processActivities(
        std::bind(&CuptiActivityProfiler::handleRocprofActivity, this,
                  std::placeholders::_1, &logger),
...
static std::optional<WaitEventInfo> getWaitEventInfo(uint32_t ctx,
                                                     uint32_t eventId) {
...
void CuptiActivityProfiler::handleCudaSyncActivity(
  ...
  auto log_event = [activity, src_stream, src_corrid, device_id, logger,
                    this]() {
    const ITraceActivity *linked =
...
inline void CuptiActivityProfiler::handleGpuActivity(const rocprofAsyncRow *act,
                                                     ActivityLogger *logger) {
...
void CuptiActivityProfiler::handleRocprofActivity(const rocprofBase *record,
                                                  ActivityLogger *logger) {
  switch (record->type) {
  case ROCTRACER_ACTIVITY_DEFAULT:
    handleRuntimeActivity(reinterpret_cast<const rocprofRow *>(record), logger);
    break;
  case ROCTRACER_ACTIVITY_KERNEL:
    handleRuntimeActivity(reinterpret_cast<const rocprofKernelRow *>(record),
                          logger);
    break;
  case ROCTRACER_ACTIVITY_COPY:
    handleRuntimeActivity(reinterpret_cast<const rocprofCopyRow *>(record),
                          logger);
    break;
  case ROCTRACER_ACTIVITY_MALLOC:
    handleRuntimeActivity(reinterpret_cast<const rocprofMallocRow *>(record),
                          logger);
    break;
  case ROCTRACER_ACTIVITY_ASYNC:
    handleGpuActivity(reinterpret_cast<const rocprofAsyncRow *>(record),
                      logger);
    break;
...
void CuptiActivityProfiler::performMemoryLoop(const string &path,
