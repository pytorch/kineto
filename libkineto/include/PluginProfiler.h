...
// This file handles pure C plugin profiler interface and converts to internal profiler interface

class PluginProfilerSession : public IActivityProfilerSession {

public:
    PluginProfilerSession(const KinetoPlugin_ProfilerInterface &profiler) : profiler_(profiler)
    {
        int errorCode = 0;
        
        char profilerName[32];
        KinetoPlugin_ProfilerQuery_Params queryParams{KINETO_PLUGIN_PROFILER_QUERY_PARAMS_UNPADDED_STRUCT_SIZE}; 
        ...
        KinetoPlugin_ProfilerCreate_Params createParams{KINETO_PLUGIN_PROFILER_CREATE_PARAMS_UNPADDED_STRUCT_SIZE};
        errorCode = profiler_.profilerCreate(&createParams);
        if (errorCode != 0)
        {
            LOG(ERROR) << "Plugin profiler " << name_ << " failed at profilerCreate() with error " << errorCode;
    ...
    ~PluginProfilerSession()
    {
        ...
        KinetoPlugin_ProfilerDestroy_Params destroyParams{KINETO_PLUGIN_PROFILER_DESTROY_PARAMS_UNPADDED_STRUCT_SIZE};
        destroyParams.pProfilerHandle = pProfilerHandle_;
        errorCode = profiler_.profilerDestroy(&destroyParams);
        if (errorCode != 0)
        {
            LOG(ERROR) << "Plugin profiler " << name_ << " failed at profilerDestroy() with error " << errorCode;
        }
    }
    ...
    void start() override 
    {
        lastStartTimestampUtcNs_ = std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
        ...
        KinetoPlugin_ProfilerStart_Params startParams{KINETO_PLUGIN_PROFILER_START_PARAMS_UNPADDED_STRUCT_SIZE};
        startParams.pProfilerHandle = pProfilerHandle_;
        errorCode = profiler_.profilerStart(&startParams);
        if (errorCode != 0)
        {
            LOG(ERROR) << "Plugin profiler " << name_ << " failed at profilerStart() with error " << errorCode;
        }
    }
    ...
    void stop() override
    {
        lastStopTimestampUtcNs_ = std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
        ...
        KinetoPlugin_ProfilerStop_Params stopParams{KINETO_PLUGIN_PROFILER_STOP_PARAMS_UNPADDED_STRUCT_SIZE};
        stopParams.pProfilerHandle = pProfilerHandle_;
        errorCode = profiler_.profilerStop(&stopParams);
        if (errorCode != 0)
        {
            LOG(ERROR) << "Plugin profiler " << name_ << " failed at profilerStop() with error " << errorCode;
        }
    }
    ...
    void processTrace(ActivityLogger& logger) override
    {
        ...
        auto traceSpan = TraceSpan(lastStartTimestampUtcNs_, lastStopTimestampUtcNs_, name_);
        PluginTraceBuilder pluginTraceBuilder{traceSpan};
        const KinetoPlugin_TraceBuilder traceBuilder = pluginTraceBuilder.toCTraceBuilder();
    
        int errorCode = 0;

        KinetoPlugin_ProfilerProcessEvents_Params profilerProcessEventsParams{KINETO_PLUGIN_PROFILER_PROCESS_EVENTS_PARAMS_UNPADDED_STRUCT_SIZE};
        ...
        if (errorCode != 0)
        {
            LOG(ERROR) << "Plugin profiler " << name_ << " failed at profilerProcessEvents() with error " << errorCode;
        }
...
public:
    PluginProfiler(const KinetoPlugin_ProfilerInterface &profiler) : profiler_(profiler)
    {
        validateProfiler();
    ...
    {
        // [TODO] Fix below
        static const std::set<ActivityType> supported{ActivityType::CUDA_PROFILER_RANGE};
        return supported;
    }

    std::unique_ptr<IActivityProfilerSession> configure(const std::set<ActivityType>& activity_types, const Config& config) override
    {   
    ...
    std::unique_ptr<IActivityProfilerSession> configure(int64_t ts_ms, int64_t duration_ms, const std::set<ActivityType>& activity_types, const Config& config) override
    {
...
private:
    void validateProfiler()
    {
        ...
        if (profiler_.unpaddedStructSize < KINETO_PLUGIN_PROFILER_INTERFACE_UNPADDED_STRUCT_SIZE)
        {
            LOG(ERROR) << "Plugin profiler has an incompatible version";

            profiler_.profilerCreate = [](struct KinetoPlugin_ProfilerCreate_Params *) { return -1; }; 
            profiler_.profilerDestroy = [](struct KinetoPlugin_ProfilerDestroy_Params *) { return -1; };
            profiler_.profilerQuery = [](struct KinetoPlugin_ProfilerQuery_Params *) { return -1; };
            profiler_.profilerStart = [](struct KinetoPlugin_ProfilerStart_Params *) { return -1; };
            profiler_.profilerStop = [](struct KinetoPlugin_ProfilerStop_Params *) { return -1; };
            profiler_.profilerProcessEvents = [](struct KinetoPlugin_ProfilerProcessEvents_Params *) { return -1; };
        }
    }
